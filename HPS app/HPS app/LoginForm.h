...
namespace HPSapp {
	...
	private: System::Void loginButton_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		// Start session
		int userID = dbManager->GetUserID(unmanaged_username);
		int patientID = dbManager->GetPatientID(userID);
		System::String^ managedRole = gcnew System::String(role.c_str());
		SessionManager::StartSession(	patientID > 0 ? patientID : userID,	username,managedRole);
