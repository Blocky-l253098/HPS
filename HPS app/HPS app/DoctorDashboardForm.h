...
namespace HPSapp {
	...
	private:
		void InitializeComponent(void)
		{
			...
			this->prescribeButton = (gcnew System::Windows::Forms::Button());
			...
			// prescribeButton
			this->prescribeButton->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(255)), static_cast<System::Int32>(static_cast<System::Byte>(165)), static_cast<System::Int32>(static_cast<System::Byte>(0)));
			this->prescribeButton->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 10, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->prescribeButton->ForeColor = System::Drawing::Color::White;
			this->prescribeButton->Location = System::Drawing::Point(20, 470);
			this->prescribeButton->Name = L"prescribeButton";
			this->prescribeButton->Size = System::Drawing::Size(150, 40);
			this->prescribeButton->TabIndex = 5;
			this->prescribeButton->Text = L"Write Prescription";
			this->prescribeButton->UseVisualStyleBackColor = false;
			this->prescribeButton->Click += gcnew System::EventHandler(this, &DoctorDashboardForm::prescribeButton_Click);
			...
			this->appointmentsGridView->TabIndex = 7;
	...
	private: System::Void prescribeButton_Click(System::Object^ sender, System::EventArgs^ e) {
		if (dbManager->Connect()) {
			String^ patIdStr = InputBox("Enter Patient ID:");
			if (String::IsNullOrEmpty(patIdStr)) {
				dbManager->Disconnect();
				return;
			}

			String^ medDetails = InputBox("Enter Medicine Details:");
			if (String::IsNullOrEmpty(medDetails)) {
				dbManager->Disconnect();
				return;
			}

			String^ instructions = InputBox("Enter Instructions:");
			if (String::IsNullOrEmpty(instructions)) {
				dbManager->Disconnect();
				return;
			}

			std::string pId = msclr::interop::marshal_as<std::string>(patIdStr);
			std::string meds = msclr::interop::marshal_as<std::string>(medDetails);
			std::string inst = msclr::interop::marshal_as<std::string>(instructions);
			std::string dId = std::to_string(currentUser->getUserID());
			string sqlQuery = "INSERT INTO Prescriptions (PatientID, DoctorID, MedicineDetails, Instructions) VALUES (" +
				pId + ", " + dId + ", '" + meds + "', '" + inst + "');";

			char* errMsg = nullptr;
			if (sqlite3_exec(dbManager->getDB(), sqlQuery.c_str(), nullptr, 0, &errMsg) == SQLITE_OK) {
				MessageBox::Show("Prescription added successfully!", "Success", MessageBoxButtons::OK, MessageBoxIcon::Information);
			}
			else {
				MessageBox::Show("Failed to add prescription.", "Error", MessageBoxButtons::OK, MessageBoxIcon::Error);
			}
